...
def calculate_pseudostates(
    ...
    where :math:`\\theta_{\\text{full}}` is the Aalen Johansen estimate based on the full
    sample, and :math:`\\theta_{(-i)}` is the estimate obtained after removing
    individual :math:`i`.
    ...
    See Also
    --------
    polarstate.prepare_event_table : Convert raw data into an event table.
    polarstate.predict_aj_estimates : Predict Aalen-Johansen estimates.
    ...
    References
    ----------
    .. [1] Andersen, P. K., & Pohar Perme, M. (2010).
    ...
    # Pre-calculate the full sample term
    theta_full_numeric = theta_full.select(numeric_cols)
    scaled_theta_full = theta_full_numeric * n

    # Efficiently create leave-one-out datasets
    loo_datasets = [
        times_and_reals.slice(0, i).vstack(times_and_reals.slice(i + 1))
        for i in range(n)
    ]

    # Process leave-one-out estimates in a batch
    loo_estimates = [
        predict_aj_estimates(
            prepare_event_table(sub_data),
            fixed_time_horizons=pl.Series([fixed_time_horizon]),
        ).select(numeric_cols)
        for sub_data in loo_datasets
    ]

    # Calculate pseudo-observations
    pseudo_rows = [
        (
            theta_full.select(id_cols)
            .with_columns(
                [
                    (
                        scaled_theta_full.get_column(c)
                        - (n - 1) * loo_estimates[i].get_column(c)
                    ).alias(c)
